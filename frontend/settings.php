
	<div class="h-screen md:w-full md:max-w-6xl min-w-0 mx-auto px-6">
  <div x-data="app()">
		<h1 class="text-lg md:text-2xl text-gray-900 font-semibold">Account Settings</h1>
		<div class="mt-6 flex md:overflow-x-auto md:scrollbar-none" style="box-shadow: rgb(237, 242, 247) 0px -2px 0px inset;">
    <a x-text="tab" class="text-gray-900 border-gray-800 undefined block pb-4 font-semibold text-sm sm:text-base border-b-2 focus:outline-none focus:text-gray-900 whitespace-no-wrap" href="/account-settings/basic-information">
    Basic Information
    </a>
    <a class="text-gray-600 hover:text-gray-700 border-transparent ml-10 block pb-4 font-semibold text-sm sm:text-base border-b-2 focus:outline-none focus:text-gray-900 whitespace-no-wrap" href="/account-settings/profile">
    Profile
    </a>
    </div>
		<p class="text-sm text-gray-700 my-6">Help us personalize your experience and make big account changes here by setting your login preferences.</p>
			<div class="flex mt-12 bg-white rounded-md ">
				<div class="md:w-64">
					<div class=""> <a :class="{ 'border-gray-400 text-gray-400 disabled': tab === 'Username' }" @click.prevent="tab = 'Username'" class="cursor-pointer block hover:text-gray-400 hover:border-gray-400 transition py-2 px-4 md:py-3 md:px-6 font-semibold border-l">
							Username
						</a>
            <a :class="{ 'border-gray-400 text-gray-400 disabled': tab === 'Profile' }" @click.prevent="tab = 'Profile'" class="cursor-pointer block hover:text-gray-400 hover:border-gray-400 transition py-2 px-4 md:py-3 md:px-6 font-semibold border-l">
            Profile
						</a>            
             <a :class="{ 'border-gray-400 text-gray-400': tab === 'Security' }" @click.prevent="tab = 'Security'" class="cursor-pointer block hover:text-gray-400 hover:border-gray-400 transition py-2 px-4 md:py-3 md:px-6 font-semibold border-l">
							Security
						</a> <a :class="{ 'border-gray-400 text-gray-400': tab === 'Integrations' }" @click.prevent="tab = 'Integrations'" class="cursor-pointer block hover:text-gray-400 hover:border-gray-400 transition py-2 px-4 md:py-3 md:px-6 font-semibold border-l">
							Integrations
						</a>
						<div class="my-auto"> <a :class="{ 'border-gray-400 text-gray-400': tab === 'Logout' }" @click.prevent="tab = 'Logout'" class="cursor-pointer block hover:text-gray-400 hover:border-gray-400 transition py-2 px-4 md:py-3 md:px-6 font-semibold border-l">
							Log Out
						</a> </div>
					</div>
				</div>
				<div class="flex-grow">
					<div class="flex flex-col mx-5 my-4">
         <div class="mb-4"> <?php
/*Call our notification handling*/ include("../frontend/sitenotif.php");
?>          </div>
						<form action="" method="post">
							<div x-show="tab === 'Username'">
								<?php include('components/changeusername/changeusername.php'); ?>
							</div>
							<div x-show="tab === 'Profile'">
								<?php include('components/addavatar/addavatar.php'); ?>
							</div>              
							<div x-show="tab === 'Security'">
								<?php include('components/changepassword/changepassword.php'); ?>
							</div>
							<div x-show="tab === 'Integrations'">
								<?php include('components/addsteam/addsteam.php'); ?>
							</div>
							<div x-show="tab === 'Logout'">
								<?php include('components/logout/logout.php'); ?>
							</div>
					</div>
				</div>
			</div>
		</div>
		<div class="my-4 ">
			<div class="my-auto px-0 py-5 flex justify-end">
				<button type="button" class="px-4 py-3 leading-none font-semibold rounded-lg bg-gray-300 text-gray-900 hover:bg-gray-400 focus:outline-none focus:bg-gray-400">Cancel</button>
				<button type="submit" name="settings" class="ml-4 px-6 py-3 leading-none font-semibold rounded-lg bg-gray-800 text-white hover:bg-gray-900 focus:outline-none focus:bg-gray-900">Save</button>
			</div>
			</form>
		</div>
	</div>
	<?php if(isset($success)): ?>
		<div class="pt-8">
			<div class="bg-green-200 border-l-4 border-green-300 text-green-800 p-4">
				<p class="font-bold">Success!</p>
				<p>
					<?php switch($success){case "username": echo('Your username has been changed.'); break; case "password": echo('Your password has been changed.'); break; default: break;}?>
				</p>
			</div>
		</div>
		<?php endif; ?>
    <script>
		function app() {
			return {
				tab: 'Username', 
				image: 'data:image/jpeg;base64,',
			}
		}
	</script>